...
def basicModifyProductData():
	productID = makeLdMod(input("Product id: "))
	productUrl = fldrUrlMasterDatabase +"/"+ str(productID)
	if os.path.exists(productUrl):
		#PRODUCT CHECK
		prCheckUrl = fldrUrlMasterDatabase +"/"+ str(productID) +"/"+ fldrFileProduct
		...
		if it8c.fileTextExists(refPlateUrl) == 1:
			...
			if checkb == 1:
				...
				if checka < pageLogAb:
					...
					checkd = fldrUrlMasterDatabase +"/"+ str(productID) +"/"+ fldrFileDataContent #DATA FILE
